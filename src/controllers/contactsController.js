class ContactsController {
    ...
    async createContact(req, res) {
        const { first_name, last_name, business, email, phone_type, phone, website } = req.body;
        try {
            const result = await this.db.query(
                'INSERT INTO contacts (first_name, last_name, business, email, phone_type, phone, website) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING *',
                [first_name, last_name, business, email, phone_type, phone, website]
            );
    ...
    async updateContact(req, res) {
        ...
        try {
            const result = await this.db.query(
                ...
                [first_name, last_name, business, email, phone_type, phone, website, id]
            );
            if (result.rows.length === 0) {
                return res.status(404).json({ error: 'Contact not found' });
            }
